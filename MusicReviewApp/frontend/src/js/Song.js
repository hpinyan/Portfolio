...
function Song() {
    ...
    const handleSubmit = (event) => {
      if(rate < 0 && rate > 10) {
        ...
        if(prev) {
          ...
          navigate("/error", { state: { err: "Post Review Error" } });
        })
        }
      }
    };
    ...
    const handleRateChange = (event) => {
      ...
      useEffect(() => {
        ...
        api.getByMbid(type, id).then(l => {
          ...
          navigate("/error", { state: { err: "Get By Mbid Error --song" } });
          setLoading(false);
        })
    }, [id, navigate])
    ...
    const sendToAlbum = () => {
      navigate(`/album?id=${track.album.id}`);
    };
            ...
            <center>
              <div className="card song-card">
                ...
                <div className="card-body ">
                  <div className="card-text song-card-title">{track.name} <i onClick={() => {        
                    setSModalOpen(true);
                  }} className="plus-icon bi bi-plus"></i></div>
                  {sModalOpen && <AddSongModal track = {track} setSOpenModal={setSModalOpen} />}
                  {sModalOpen && <div className="modalOverlay" />}
                  <div
                    className="btn btn-secondary m-1 card-text"
                    onClick={sendToArtist}
                  >
                  ...
                  <button type="button" className="btn btn-tertiary ">
                    ...
                    <span className="badge text-bg-warning">
                      {reviewState ? reviewState.score : "N/A"}
                    </span>
                  </button>
                  <br />
                </div>
